...
- (void)viewDidLoad {
    myOtherRef = [[Firebase alloc] initWithUrl:@"https://intense-inferno-6787.firebaseio.com/items"];
    ...
    [myOtherRef observeEventType:FEventTypeChildAdded withBlock:^(FDataSnapshot *snapshot) {
        ...
        if([itemType  isEqual: @"others"]){
            
            if(self.otherItems.count < 1000) {
            [self.otherItems addObject:newItem];
            }
            [self.otherItems sortUsingFunction:compareDateOthers context:nil];
        }
...
NSComparisonResult compareDateOthers(id num1, id num2, void* context){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Item* itemForRow = self.otherItems[indexPath.row];
