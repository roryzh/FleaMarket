...
@property (nonatomic, strong) NSMutableArray* rideItems;
...
- (void)viewDidLoad {
    myRideRef = [[Firebase alloc] initWithUrl:@"https://intense-inferno-6787.firebaseio.com/items"];
    ...
    self.rideItems = [[NSMutableArray alloc] init];
    [myRideRef observeEventType:FEventTypeChildAdded withBlock:^(FDataSnapshot *snapshot) {
        ...
        if([itemType  isEqual: @"ride"]){
            
            if(self.rideItems.count < 1000){
            [self.rideItems addObject:newItem];
            }
            [self.rideItems sortUsingFunction:compareDateRide context:nil];
        }
...
NSComparisonResult compareDateRide(id num1, id num2, void* context){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Item* itemForRow = self.rideItems[indexPath.row];
