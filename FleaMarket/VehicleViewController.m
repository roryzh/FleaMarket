...
@property (nonatomic, strong) NSMutableArray* vehicleItems;


@end

@implementation VehicleViewController
Firebase *myVehicleRef;
Item* myVehicleItem;


- (void)viewDidLoad {
    myVehicleRef = [[Firebase alloc] initWithUrl:@"https://intense-inferno-6787.firebaseio.com/items"];
    ...
    self.vehicleItems = [[NSMutableArray alloc] init];
    [myVehicleRef observeEventType:FEventTypeChildAdded withBlock:^(FDataSnapshot *snapshot) {
        ...
        if([itemType  isEqual: @"vehicle"]){
            
            if(self.vehicleItems.count < 1000){
            [self.vehicleItems addObject:newItem];
            }
            [self.vehicleItems sortUsingFunction:compareDateVehicle context:nil];
        }
...
NSComparisonResult compareDateVehicle(id num1, id num2, void* context){
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return [self.vehicleItems count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Item* itemForRow = self.vehicleItems[indexPath.row];
